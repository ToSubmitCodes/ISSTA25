workspace = './testWorkSpace/noFail_workspace'
popSize = 50
...
def BackendTest(backend,round):          
    ...
    stopflag.touch()
    dill.dump(allRes,open(f'../results/RQ2/res_{backend}_noFail_{round}.pickle','wb'))
    dill.dump(failCore,open(f'../results/RQ2/core_{backend}_noFail_{round}.pickle','wb'))
    json.dump(actParams,open(f'../results/RQ2/ap_{backend}_noFail_{round}.json','w'),indent=6)
