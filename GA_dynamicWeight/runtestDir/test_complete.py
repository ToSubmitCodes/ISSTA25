workspace = './testWorkSpace/complete_workspace'
popSize = 50
...
def score(seedpool,allRes,argInfoList,keyParams):
    ...
    for testcase in seedpool:
        s = 1
        for i,v in enumerate(testcase):
            base = 1.0*keyParams[i] if i in keyParams.keys() else 1.0
...
def mutate(testCase,argInfoList,keyParams):
    ...
    for i,val in enumerate(testCase):
        mutationPossibility = 0.05 if i not in keyParams.keys() else 0.05 * keyParams[i]
...
def clsTest_ActGen(backend,clsName, paramInfo, seedpool):
    ...
    controlParams = extractControl(paramInfo,clsName)
    shapeParams = initShape(SeedPool,allRes)
    ActiveParams = {p:2.0 for p in (controlParams|shapeParams)}
    APRecord = {'Control':controlParams,'Shape':shapeParams,'Fail':set()}
    ScoreDict = score(SeedPool,allRes,argInfoList,ActiveParams)
    #start testing
    roundCnt = 0
    while(len(allRes) < maxIter and roundCnt < 100):
        print('Round: ',roundCnt)
        roundCnt += 1
        ActiveParams = {p:ActiveParams[p]-0.1 if ActiveParams[p]>1.0 else 1.0 for p in ActiveParams}
        #crossover & mutation
        nextGeneration = [] 
        newPop = []
        tryCnt = 0
        while(len(nextGeneration) < popSize and tryCnt < 2000):
            ...
            diffPSet = [paramIndex for paramIndex in ActiveParams.keys() if p1[paramIndex] != p2[paramIndex] ]
            if len(diffPSet) > 0:
            ...
            for p in newP1,newP2:
                newP = mutate(p,argInfoList,ActiveParams)
                if valid(newP,allFailCores.values()) : #and valid(newP,allShapeCores.values()):
        ...
        for tc in nextGeneration:
            ...
            if resSumy != 'Crash':
                for oldTc in allRes.keys():
                    ...
                    if shapeParam is not None:
                        ActiveParams[shapeParam] = 2.0
                        APRecord['Shape'].add(shapeParam)
            else:
                ...
                for oldTc in allRes.keys():
                    ...
                    if shapeParam is not None:
                        ActiveParams[shapeParam] = 2.0
                        APRecord['Fail'].add(shapeParam)
            
        ScoreDict = score(newPop,allRes,argInfoList,ActiveParams)

 
    print('#Test Cases: ',len(allRes))
    APRecord = {k:list(v) for k,v in APRecord.items()}
    return allRes,{},APRecord
...
def BackendTest(backend,round):          
    ...
    stopflag.touch()
    dill.dump(allRes,open(f'../results/RQ2/res_{backend}_complete_{round}.pickle','wb'))
    dill.dump(failCore,open(f'../results/RQ2/core_{backend}_complete_{round}.pickle','wb'))
    json.dump(actParams,open(f'../results/RQ2/ap_{backend}_complete_{round}.json','w'),indent=6)
